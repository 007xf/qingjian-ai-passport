...
- 减少后台唤醒，支持每分钟/每小时同步与静默界面刷新；工牌离线超过90秒后释放保持唤醒请求。电量计经完整验证为空白时仅初始化一次既有520mAh参数，保留所有非空参数。

- 新增可选择启用的 Mac 后台同步服务，由单一服务持有 USB／蓝牙连接，对已选工牌逐步延长间隔重连，保留上次读数时间，并提供可选的熄屏保持唤醒设置。成长值改为真实 Codex 当前周期 Token 加 Cursor 账户当月 Token，按各来源口径包含缓存输入，来源不可用不会被当成零。详见[后台同步与成长计数](qingjian-background-sync.zh_CN.md)，其中说明图片仅 USB 更新、系统睡眠限制及仅支持 Mac 的范围；新增实机验收与主机测试仍分别报告。

- 从 Mac 功能与来源设置、工牌导航中移除 Grok Bot，保留原协议位及独立 Mac 应用。可选项为 Codex、Cursor、Gemini、智能体、Dino；智能体行也随相应 AI 来源的勾选隐藏或显示。
- Dino 飞鸟碰撞改为 Chromium 98 固定身体框，替代逐动画像素判定，中层飞鸟翅尖不再使持续下蹲误判撞击。主机回归覆盖全部振翅/下蹲组合、828 分完整穿越、低飞鸟跳跃与最高分保留；新版飞鸟行为仍待实机验收。

- 青笺 1.3.0 新增数据源检测，以及带备份的 Cursor / Gemini CLI 官方活动回调配置入口，并明确区分原生客户端接入限制。

- 青笺 1.2.0 新增 Cursor 本地代码活动计数、Gemini CLI Token 元数据与按来源区分的智能体状态。官方 hooks 只保存有界匿名活动；私有 Codex 增量事件索引先确认实际任务开始，再把进度视为工作中。缺失、覆盖不完整和过期会明确显示，Mac 检查固件支持。安装合并既有 hooks 并备份；App 移动后只更新已安装的解释器映射。阈值标签显示精确整数。
- Cursor CLI 与真实结束 hook 已通过本机验证。当前 Gemini 个人 CLI 请求被服务端拒绝，没有新增 Token；Grok 桌面应用仍无可用的外部状态桥，不把这些限制写成完全连通。
- Dino 改用 Chromium 原版精灵图集，保留可复现转换与 BSD 来源说明；造型和操作已通过用户实机确认。蓝牙使用、仅 USB 的图片上传/恢复与断电保存、无重置 USB 重连以及保护区复读均已本地验证。最终 1.2.0 App 构建、深度签名验证及 200 项内置运行环境测试通过；新增看板界面与设备回读仍作为独立验收步骤。

- 自定义头像写入和恢复默认图片限定通过 USB 完成。蓝牙继续同步文字、设置、时间、Token 和额度，报告待更新形态而不清空设备唯一的自定义图片缓存。青笺 1.1.1 新增离线草稿保存、图片仅 USB 的明确提示，以及避免应用 DTR/RTS 切换触发重启的 macOS 运行时串口适配器。
- 新增可编辑电子工牌、同步时间、实际电量、三阶段头像进化、可配置门槛，以及三连按 OK 熄屏与完整唤醒手势隔离。新自定义图片使用有界 APZ1 zlib 压缩 RGB565，分辨率自适应 56–128 像素，透明部分合成黑底，并提供一致的主机像素预览。
- 新增主 Codex 额度看板，按来源显示实际零、一或两个窗口，计算剩余百分比并标注来源，以源观测时间计算最长 180 秒有效期。缓存或日志中重复的快照不会因重发变新鲜；不支持或缺失的值保持未知。
- 新增可玩的 Dino，支持跳跃、按住下蹲、暂停与继续、长 OK 退出、熄屏暂停和最高分保存。独立 Coding Pet 桌宠已取消，工牌头像进化保留；AI Talk 延后，其他 AI 数据源尚未全部接通。
- 新增加密 BLE NUS，与 USB 共用 `@AP` 协议；通过设置或可信 USB 打开 120 秒配对窗口，在 Mac 输入设备显示的六位码，并实现绑定与断线会话隔离。Mac 后端要求明确选择外设 UUID，先订阅再按 MTU 分片写入，首次连接预留 60 秒，切换连接不重置 Token 周期。蓝牙实机验收仍在进行，本版本不启动 Wi-Fi 或音频。
