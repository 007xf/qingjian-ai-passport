...
esp_err_t bsp_button_init(bsp_btn_cb_t cb, void *user) {
    ...
    for (int i = 0; i < BSP_BTN_COUNT; i++) {
        ...
        // Release is required by the wake-gesture gate; never report a partial
        // callback set as ready. Existing event values remain unchanged.
        if ((e = iot_button_register_cb(s_btn[i], BUTTON_PRESS_DOWN, NULL, cb_press, idx)) != ESP_OK ||
            (e = iot_button_register_cb(s_btn[i], BUTTON_SINGLE_CLICK, NULL, cb_click, idx)) != ESP_OK ||
            (e = iot_button_register_cb(s_btn[i], BUTTON_DOUBLE_CLICK, NULL, cb_double, idx)) != ESP_OK ||
            (e = iot_button_register_cb(s_btn[i], BUTTON_LONG_PRESS_START, NULL, cb_long, idx)) != ESP_OK ||
            (e = iot_button_register_cb(s_btn[i], BUTTON_PRESS_UP, NULL, cb_release, idx)) != ESP_OK) {
            s_cb = NULL;
            for (int j = 0; j <= i; ++j) {
                if (s_btn[j]) { iot_button_delete(s_btn[j]); s_btn[j] = NULL; }
            }
            ESP_LOGE(TAG, "Button callback registration failed: %s", esp_err_to_name(e));
            return e;
        }
    }
